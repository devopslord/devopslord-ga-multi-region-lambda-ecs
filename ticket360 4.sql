...
create index sites_site_number_idx on dbo.sites (site_number);
...
(
  ...
  foreign key (pass_type_id) references pass_types(pass_type_id)
);
...
(
  ...
  foreign key (created_at_site_id) references sites(site_id),
  foreign key (guest_external_system_id) references external_systems(external_system_id)
);
