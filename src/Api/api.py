...
def create_payment_table(event, context):
    ...
    try:
        ...
        print("Error  creating payment table: " + str(error))
        traceback.print_exc()
        result = str(error)
...
def drop_payment_table(event, context):
    ...
    try:
        ...
        print("Error dropping payment table: " + str(error))
        traceback.print_exc()
        result = str(error)
...
def get_payments(event, context):
    ...
    try:
        ...
        print("Error getting payments", error)
        traceback.print_exc()
        result = str(error)
...
def create_payment(event, context):
    ...
    try:
        ...
        if input_body:
            ...
            query = """INSERT INTO payment (sender_name, sender_bank, sender_account, receiver_name, receiver_bank, receiver_account, amount, creation_timestamp) VALUES (%s, %s, %s, %s, %s, %s, %s, current_timestamp);"""
            cursor = db_connection.cursor(cursor_factory=RealDictCursor)
        ...
        print("Error creating payment" + str(error))
        traceback.print_exc()
        result = str(error)
...
def update_payment(event, context):
    ...
    try:
        ...
        if input_body:
            ...
            query = """update payment set sender_name = %s, sender_bank = %s, sender_account = %s, receiver_name = %s, receiver_bank = %s, receiver_account = %s, amount = %s  where id = %s"""
            cursor = db_connection.cursor(cursor_factory=RealDictCursor)
            cursor.execute(query, (sender_name, sender_bank, sender_account, receiver_name, receiver_bank, receiver_account, amount, payment_id))
            db_connection.commit()
            status_code = 200
        else:
            result = "Error, incorrect post body"
        ...
        print("Error updating payment" + str(error))
        traceback.print_exc()
        result = str(error)
...
def delete_payment(event, context):
    ...
    try:
        ...
        print("Error deleting payment" + str(error))
        traceback.print_exc()
        result = str(error)
...
def clear_payments(event, context):
    ...
    try:
        ...
        print("Error clearing payment table: " + str(error))
        traceback.print_exc()
        result = str(error)
...
class Payment:
    """
    It represents a payment.
    """

    def __init__(self,
                 record):
        """
        Initializes the payment from the database record.
